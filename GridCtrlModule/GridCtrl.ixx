...
//#define DDX_GridControl(pDX, nIDC, rControl)  DDX_Control(pDX, nIDC, rControl)     
	...
	inline void AFXAPI DDX_GridControl(CDataExchange* pDX, int nIDC, CWnd& rControl)
	{
	...
	{
		GVL_NONE = 0L,      // Neither
		GVL_HORZ = 1L,      // Horizontal line or scrollbar
		GVL_VERT = 2L,      // Vertical line or scrollbar
		GVL_BOTH = 3L      // Both
	};
...
// #define GVL_BOTH                3L      // Both
	...
	{
		GVS_DEFAULT = 0,
		GVS_HEADER = 1,       // Size using column fixed cells data only
		GVS_DATA = 2,       // Size using column non-fixed cells data only
		GVS_BOTH = 3       // Size using column fixed and non-fixed
...
// Cell Searching options
	...
	{
		GVNI_FOCUSED = 0x0001,
		GVNI_SELECTED = 0x0002,
		GVNI_DROPHILITED = 0x0004,
		GVNI_READONLY = 0x0008,
		GVNI_FIXED = 0x0010,
		GVNI_MODIFIED = 0x0020,

		GVNI_ABOVE = LVNI_ABOVE,
		GVNI_BELOW = LVNI_BELOW,
		GVNI_TOLEFT = LVNI_TOLEFT,
		GVNI_TORIGHT = LVNI_TORIGHT,
		GVNI_ALL = (LVNI_BELOW | LVNI_TORIGHT | LVNI_TOLEFT),
		GVNI_AREA = (LVNI_BELOW | LVNI_TORIGHT)
...
// Hit test values (not yet implemented)
	enum HitTestValues
	{
		...
		GVHT_COLHDR = 0x0002,
		GVHT_ROWHDR = 0x0004,
		GVHT_COLSIZER = 0x0008,
		GVHT_ROWSIZER = 0x0010,
		GVHT_LEFT = 0x0020,
		GVHT_RIGHT = 0x0040,
		GVHT_ABOVE = 0x0080,
		GVHT_BELOW = 0x0100
...
// Messages sent to the grid's parent (More will be added in future)
	enum NotificationsToParent
	{
		GVN_BEGINDRAG = LVN_BEGINDRAG,        // LVN_FIRST-9
		GVN_BEGINLABELEDIT = LVN_BEGINLABELEDIT,   // LVN_FIRST-5
		GVN_BEGINRDRAG = LVN_BEGINRDRAG,
		GVN_COLUMNCLICK = LVN_COLUMNCLICK,
		GVN_DELETEITEM = LVN_DELETEITEM,
		GVN_ENDLABELEDIT = LVN_ENDLABELEDIT,     // LVN_FIRST-6
		GVN_SELCHANGING = LVN_ITEMCHANGING,
		GVN_SELCHANGED = LVN_ITEMCHANGED,
		GVN_GETDISPINFO = LVN_GETDISPINFO,
		GVN_ODCACHEHINT = LVN_ODCACHEHINT
...
// #define GVN_ODCACHEHINT         LVN_ODCACHEHINT 
	...
	constexpr auto GRIDCTRL_CLASSNAME = _T("MFCGridCtrl");  // Window class name
//#define IDC_INPLACE_CONTROL   8                  // ID of inplace edit controls
	constexpr auto IDC_INPLACE_CONTROL = 8;
	...
	public:
		...
		///////////////////////////////////////////////////////////////////////////////////
	public:
#ifndef  DO_NOT_ADD_JD_EXTENSIONS
		std::vector<PFNLVCOMPARE> m_sortingFunctions;
		// void SetSortingFor(int col, PFNLVCOMPARE pfnCompare);
#endif
	...
	protected:
		...
		afx_msg void OnTimer(UINT_PTR  /* UINT*/ nIDEvent);		// JD changed
...
#ifndef _WIN32_WCE
#ifdef DO_NOT_ALLOW_JD_TO_COMMENT_OUT_WIN95_SUPPORT
...
constexpr size_t LAYER_SIGNATURE = 0x5FD4E64;
...
void CGridCtrl::OnTimer(UINT_PTR /*UINT*/ nIDEvent)
{
...
#if 0 // JD
GRIDCTRL_API void CGridCtrl::SetSortingFor(int col, PFNLVCOMPARE pfnCompare)
{
    if (m_sortingFunctions.size() < col)
    {
        m_sortingFunctions.re
    }
}
#endif
...
#ifndef  DO_NOT_ADD_JD_EXTENSIONS
    if (m_sortingFunctions.size() > static_cast<size_t>(nCol))
    {
        auto pfnCompare = m_sortingFunctions[nCol];
        return CGridCtrl::SortItems(pfnCompare, nCol, bAscending, data);
    }
#endif
...
#define JDH
#ifdef JDH
                        auto x = CellRect.TopLeft().x;
                        auto y = CellRect.TopLeft().y;
                        auto width = CellRect.Width();
                        auto height = CellRect.Height();
                        auto hwnd = m_TitleTip.m_hWnd;
                        auto ret = ::SetWindowPos(hwnd, wndTopMost.m_hWnd, x, y, width, height, SWP_SHOWWINDOW);
#endif
