...
{
#define JDH_TESTING_TOOLTIPS
#ifdef JDH_TESTING_TOOLTIPS
	auto red = GetRValue(crTextClr);
	auto green = GetGValue(crTextClr);
	auto blue = GetBValue(crTextClr);

	auto bred = GetRValue(crBackClr);
	auto bgreen = GetGValue(crBackClr);
	auto bblue = GetBValue(crBackClr);

	//crTextClr = 0x00FF0000;
	//crBackClr = 0x0000FFFF;
